...
void Ultrasonic::DistanceMeasure(void)
{
  ...
  digitalWrite(_pin, LOW);
  pinMode(_pin, INPUT);
  durationOnePin = pulseIn(_pin, HIGH);
}
/*The measured distance from the range 0 to 400 Centimeters*/
float Ultrasonic::microsecondsToMillimeters(void)
{
  return durationOnePin *10/ 29 / 2;
}

/*The measured distance from the range 0 to 157 Inches*/
float Ultrasonic::microsecondsToInches(void)
{
  return durationOnePin / 74 / 2;
}
